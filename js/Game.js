class Game {
  ...
  init(canvas) {
    ...
    for (let i = 0; i < this.START_BALLS_COUNT; i++) {
      this.balls.push(new Ball(this.context, this.range));
    }
    this.needle = new Needle(this.context, this.range);
  ...
  startGame(callback) {
    this.timerID = setInterval(() => {
      ...
      this.ballsNumbers = Math.round(
        this.START_BALLS_COUNT + this.increaseRatio * 2
      );
  ...
  hintObserver(ball) {
    if (
      (this.needle.tipCord.x - ball.x) ** 2 +
        (this.needle.tipCord.y - ball.y) ** 2 <=
      ball.r ** 2
    ) {
      this.ballToggle(ball);
      this._hints++;
  ...
  ballToggle(ball) {
    ...
    if (this.gameTime < this.TIMER) {
      while (this.balls.length < this.ballsNumbers)
        this.balls.push(new Ball(this.context, this.range));
    }
  }
